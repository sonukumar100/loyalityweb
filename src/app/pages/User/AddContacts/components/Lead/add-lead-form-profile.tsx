...
export function AddLeadFormProfile() {
  ...
  const makeCall = () => {
    if (editSelector?.phone) {
      dispatch(
        globalActions.setCalling({
          status: 'dialing',
          contact: {
            phone: editSelector.phone,
            fullName: editSelector.fullName,
            avatar: editSelector.avatar,
          },
        }),
      );
      setCallTriggered(true); // Update the state to indicate that makeCall was triggered
      // Here you can add the logic to make the call
    }
  };

  return (
    <>
      {/* <Calls leadData={editSelector} callTriggered={callTriggered} resetCallTrigger={resetCallTrigger} /> */}
      <div className="rounded-24 bg-white py-10 px-10">
          ...
          <div className="relative flex items-center">
            ...
            <div className="ml-10">
              ...
              <p className="flex mt-2">
                ...
                <span className="text-[#12B76A] text-12 font-poppins font-normal ml-2">
                  {settingConfig.clientStatus.find(
                    f => f.key == form.watch('clientStatuses'),
                  )?.value || '_'}
                </span>
              </p>
            </div>
        ...
        <div className="grid grid-cols-3 items-center w-[100%]  mt-12">
          <div className="relative max-w-[15rem]">
            <div className=" before:content-[''] before:absolute before:top-[.6rem] before:w-[.1rem] before:-right-[0rem] before:h-[3rem] before:bg-[#EAECF0]">
              ...
              <h2 className="text-primary text-base font-poppins font-medium mt-4">
                {settingConfig.leadSource.find(
                  f => f.key == form.watch('leadSource'),
                )?.value || '_'}
              </h2>
            </div>
          </div>
          <div className="relative max-w-[10.5rem]">
            <div className=" before:content-[''] before:absolute before:top-[.6rem] before:w-[.1rem] before:-right-[0rem] before:h-[3rem] before:bg-[#EAECF0]">
          ...
          <div className="">
            <h6 className="font-poppins text-base text-secondary">Campaign</h6>
            <h2 className="text-primary text-base font-poppins font-medium mt-4">
              {campaignListData?.filter(
                d => d._id === form.watch('campaign'),
              )?.[0]?.title || '_'}
            </h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mt-12">
          <div>
            {' '}
            <Label className="py-2">Client Status</Label>
            <Select
              value={form.watch('clientStatuses')?.toString()}
              {...form.register('clientStatuses', { required: true })}
              onValueChange={value => {
                form.setValue('clientStatuses', value);
              }}
            >
              ...
              <SelectContent>
                <SelectGroup>
                  {settingConfig.clientStatus.map(item => (
                    <SelectItem
                      key={Number(item.key)}
            ...
            <p className="text-red-600">
              {form.formState.errors.clientStatuses?.type === 'required' &&
                !form.watch('clientStatuses') &&
                `Required`}
            </p>
          </div>
          <div>
            ...
            {/* {device && !isOnCall && <button onClick={makeCall} disabled={isCalling}>Call</button>} */}
          </div>
        </div>
      </div>
    </>
