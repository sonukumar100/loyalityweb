...
import RecruitFilter from './recruit-filter';

type RecruitsTableProps = {
  search: string;
...
const RecruitsTable = (props: RecruitsTableProps) => {
  const { search, setSearch, table, pagination, columns, recruitListData } =
    props;
                    ...
                    <TableRow key={headerGroup.id}>
                      {headerGroup.headers.map(header => {
                        return (
                          <TableHead
                            key={header.id}
                            className={`text-[1.2rem] font-medium px-8`}
                          >
            ...
            <div className="flex items-center pl-6 justify-start space-x-2 py-4">
              <div className="flex items-center pl-6 space-x-10 py-4">
                ...
                <p className="text-gray-500 font-normal text-base font-poppins">
                  ...
                  {recruitListData?.data?.totalDocuments} rows selected
                </p>
              </div>
            </div>
          </div>
