...
import { Loans } from '../../../User/AddContacts/components/Lead/lead-loans';
import { AddRecruiterFormProfile } from './add-recruiter-form-profile';
import { AddRecruitersAddress } from './add-recruiter-address-info';
import { useRecruitSlice } from '../recruitsSlice';
import { removeEmptyKeys } from 'app/components/Shared/removeEmptyKeys';
import { RecruitsNote } from './recruits-note';

export function RecruitersTab() {
  const form = useFormContext();
  const { toast } = useToast();
  const navigate = useNavigate();
  const { useAddRecruitMutation, useUpdateRecruitMutation } = useRecruitSlice();
  const { actions } = useRecruitSlice();

  const [
    addRecruit,
    {
      isLoading: isAddRecruit,
      isSuccess: isAddRecruitSuccess,
      data: isAddRecruitData,
      error: isAddRecruitError,
    },
  ...
  const [
    updateRecruit,
    {
      isLoading: isUpdateRecruit,
      isSuccess: isUpdateRecruitSuccess,
      data: isUpdateRecruitData,
      error: isUpdateRecruitError,
    },
  ...
  useEffect(() => {
    if (isAddRecruitSuccess) {
      toast({
        description: 'Recruit added successfully',
      });
      navigate('/admin/contacts/recruits');
      setOpen(true);
      console.log(' isAddBuilderData', isAddRecruitData);
      dispatch(actions.setEditRecruit({ data: isAddRecruitData?.data }));

      // handleChange('true')
    } else if (isAddRecruitError) {
      toast({
        description: `${isAddRecruitError?.[0]}`,
        variant: 'destructive',
      });
    } else if (isUpdateRecruitSuccess) {
      toast({
        description: 'Recruit updated successfully',
      });
      navigate('/admin/contacts/recruits');
    } else if (isUpdateRecruitError) {
      toast({
        description: 'Recruit update failed !',
        variant: 'destructive',
      });
    }
  }, [isAddRecruitSuccess, isAddRecruitError, isUpdateRecruit]);
  ...
  const closeEdit = () => {
    ...
    dispatch(actions.setEditRecruit({ data: null }));
